...
def perform_bootstrap_sampling(df, n_iterations=1000, subsample_size=None, random_state=42):
    ...
    results = pd.DataFrame({
        ...
        'model': ['ann'] * (len(precision_scores) + len(recall_scores) + len(f1_scores) + 
...
def plot_bootstrap_distributions(results_df, output_dir):
    ...
    for metric in metrics:
        ...
        plt.title(f'Bootstrap Distribution for {metric} (Artificial Neural Network)')
        ...
        # Save plot
        plt.savefig(os.path.join(output_dir, f'ann_bootstrap_{metric}_distribution.png'))
        plt.close()
    ...
    plt.title('Performance Metrics Distribution (Artificial Neural Network)')
    plt.xlabel('Metric')
    plt.ylabel('Value')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    ...
    # Save plot
    plt.savefig(os.path.join(output_dir, 'ann_metrics_boxplot.png'))
    plt.close()
...
def create_html_report(results_df, anova_results, output_dir):
    ...
    html_path = os.path.join(output_dir, 'ann_statistical_analysis.html')
...
<head>
    ...
    <title>Statistical Analysis Report for Artificial Neural Network</title>
    <style>
        body {{
...
<body>
    <h1>Statistical Analysis Report for Artificial Neural Network</h1>
...
"""
    ...
    for metric in metrics:
        html_content += f"""
        <img src="ann_bootstrap_{metric}_distribution.png" alt="{metric} Distribution" class="chart-image">"""
    
    html_content += f"""
    </div>
    
    <h3>Combined Metrics Distribution</h3>
    <img src="ann_metrics_boxplot.png" alt="Metrics Box Plot" class="full-width-image">
...
"""
    ...
    <h2>Conclusion</h2>
    <p>The statistical analysis confirms that the Artificial Neural Network model demonstrates robust performance in anomaly detection. 
...
def main():
    ...
    output_dir = os.path.join(base_dir, "test_predictions", "2) Significatività Statistica", "ann")
    ...
    bootstrap_results.to_csv(os.path.join(output_dir, 'ann_bootstrap_results.csv'), index=False)
