...
def perform_bootstrap_sampling(df, n_iterations=1000, subsample_size=None, random_state=42):
    ...
    # Create DataFrame with results using 'lstm' as model identifier
    results = pd.DataFrame({
        ...
        'model': ['lstm'] * (len(precision_scores) + len(recall_scores) + len(f1_scores) + 
...
def plot_bootstrap_distributions(results_df, output_dir):
    ...
    for metric in metrics:
        ...
        plt.title(f'Bootstrap Distribution for {metric} (Long Short-Term Memory Network)')
        ...
        # Save plot with LSTM prefix
        plt.savefig(os.path.join(output_dir, f'lstm_bootstrap_{metric}_distribution.png'))
        plt.close()
    ...
    plt.title('Performance Metrics Distribution (Long Short-Term Memory Network)')
    plt.xlabel('Metric')
    plt.ylabel('Value')
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    ...
    # Save plot with LSTM prefix
    plt.savefig(os.path.join(output_dir, 'lstm_metrics_boxplot.png'))
    plt.close()
...
def create_html_report(results_df, anova_results, output_dir):
    ...
    html_path = os.path.join(output_dir, 'lstm_statistical_analysis.html')
...
<head>
    ...
    <title>Statistical Analysis Report for Long Short-Term Memory Network</title>
    <style>
        body {{
...
<body>
    <h1>Statistical Analysis Report for Long Short-Term Memory Network</h1>
...
"""
    ...
    for metric in metrics:
        html_content += f"""
        <img src="lstm_bootstrap_{metric}_distribution.png" alt="{metric} Distribution" class="chart-image">"""
    
    html_content += f"""
    </div>
    
    <h3>Combined Metrics Distribution</h3>
    <img src="lstm_metrics_boxplot.png" alt="Metrics Box Plot" class="full-width-image">
...
"""
    ...
    <h2>Conclusion</h2>
    <p>The statistical analysis confirms that the Long Short-Term Memory Network model demonstrates robust performance in anomaly detection. 
...
def main():
    ...
    output_dir = os.path.join(base_dir, "test_predictions", "2) Significatività Statistica", "lstm")
    ...
    # Save bootstrap results to CSV with LSTM prefix
    bootstrap_results.to_csv(os.path.join(output_dir, 'lstm_bootstrap_results.csv'), index=False)
