...
class QwenScalabilityAnalyzer:
    ...
    def load_test_data(self, data_path=None):
        """Carica i dati di test da un file CSV o genera dati di test sintetici"""
        if data_path and os.path.exists(data_path):
            self.logger.info(f"Caricamento dati di test da {data_path}")
            ...
            # Assicurati di avere almeno 200 record
            if len(df) < 200:
                self.logger.warning(f"Il dataset contiene solo {len(df)} record, potrebbero essere insufficienti")
            
            # Bilanciamento anomalie/normali
            n_anomalies = df['actual_binary'].sum()
            self.logger.info(f"Dataset caricato: {len(df)} record, {n_anomalies} anomalie ({n_anomalies/len(df)*100:.1f}%)")
    ...
    def generate_report(self, results_df):
        ...
        with open(f"{self.output_dir}/results_table.html", "w") as f:
            f.write(html_table)
...
<body>
    ...
    <ul>
        ...
        <li><strong>Considerazioni sulla memoria</strong>: L'utilizzo di memoria cresce in modo {
        'lineare' if results_df.corr()['batch_size']['avg_memory_usage'] > 0.9 else 'sub-lineare'} con la dimensione del batch,
...
def main():
    ...
    output_dir = "qwen_scalability_analysis"
    analyzer = QwenScalabilityAnalyzer(output_dir)
    
    # Carica dati di test
    # Puoi specificare un percorso CSV se disponibile, altrimenti usa dati sintetici
    ...
    print(f"Analisi completata! Report disponibile in {output_dir}/qwen_scalability_report.html")
