...
def prepare_data_for_prediction(df, feature_columns, scaler, window_size):
    """Prepara i dati per la predizione usando l'ANN"""
    ...
    return X_scaled, timestamps

def main():
    print("=== ANN Model Prediction and Evaluation ===")
    ...
    results_dir = os.path.join("test_predictions", "ann", "results")
    ...
    X_test, timestamps = prepare_data_for_prediction(df_pred, feature_columns, scaler, window_size)
    ...
    # Salva i risultati
    results_file = os.path.join(results_dir, f'ann_predictions_{timestamp}.csv')
    ...
    metrics_file = os.path.join(results_dir, f'ann_metrics_{timestamp}.json')
