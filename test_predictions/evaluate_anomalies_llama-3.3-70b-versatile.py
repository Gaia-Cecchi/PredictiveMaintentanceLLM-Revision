...
def main():
    ...
    if not api_key:
        ...
        return
    elif len(api_key.strip()) < 10:  # Basic validation of key format
        print("ERROR: GROQ_API_KEY appears to be invalid. Please check your .env file.")
        print("The API key should be a long string starting with 'gsk_'")
        return
    ...
    print("Initializing LLM predictor with llama-3.3-70b-versatile model...")
    try:
        llm_predictor = LLMPredictor(api_key, model="llama-3.3-70b-versatile")
        # Test connection with a simple prompt
        test_response = llm_predictor.llm.invoke([HumanMessage(content="Test connection")])
        print("LLM connection test successful")
    except Exception as e:
        print(f"ERROR: Failed to initialize LLM predictor: {str(e)}")
