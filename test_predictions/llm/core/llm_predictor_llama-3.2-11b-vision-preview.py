...
class LLMPredictor:
    ...
    def __init__(self, api_key: str, model: str = "llama-3.2-11b-vision-preview"):
        ...
        Args:
            ...
            model: LLM model to use, defaults to llama-3.2-11b-vision-preview
        """
        # Configure logging
        ...
        # Optimized rate limiting parameters for llama model
        self.last_request_time = 0
        self.current_interval = 2.0    # Start with 2.0 second interval for llama
        self.min_interval = 1.0        # Minimum interval between requests
        self.max_interval = 45.0       # Maximum interval between requests
        self.backoff_factor = 1.75     # Factor to increase interval after 429s
        self.success_factor = 0.9      # Factor to decrease interval after successes
        self.consecutive_successes = 0  # Add counter initialization
        self.consecutive_failures = 0   # Add counter initialization
    ...
    def predict(self, event_time: str, compressor_data_str: str, weather_data_str: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    # Optimized parameters for llama-3.2-11b-vision-preview
                    response = self.llm.invoke(
                        messages,
                        temperature=0.1,         # Slightly higher temperature for llama
                        max_tokens=200,          # Increased max tokens for llama
                        stop=["CLASSIFICAZIONE:"],
                        top_p=0.1,              # Slightly higher top_p for llama
