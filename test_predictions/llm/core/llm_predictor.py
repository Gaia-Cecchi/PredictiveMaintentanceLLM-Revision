...
# Fix the import to use absolute import rather than relative import
try:
    # Try direct import first
    from test_predictions.llm.prompts.expert_prompts import definitive_prompt
except ImportError:
    # Fallback approach - add parent directories to path
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)  # llm directory
    grandparent_dir = os.path.dirname(parent_dir)  # test_predictions directory
    ...
    try:
        ...
        prompts_path = os.path.join(parent_dir, "prompts", "expert_prompts.py")
        if os.path.exists(prompts_path):
            import importlib.util
            spec = importlib.util.spec_from_file_location("expert_prompts", prompts_path)
            expert_prompts = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(expert_prompts)
            definitive_prompt = expert_prompts.definitive_prompt
        else:
            # Define a simple fallback if all else fails
            def definitive_prompt():
                return "Analyze the compressor data and classify as ANOMALY or NORMAL VALUE."
...
class LLMPredictor:
    ...
    def __init__(self, api_key: str, model: str = "llama-3.3-70b-versatile"):
        ...
        Args:
            ...
            model: LLM model to use, defaults to llama-3.3-70b-versatile
        """
        # Configure logging
        ...
        # Initialize the LLM with robust API key handling
        ...
        try:
            ...
            # Initialize client with adjusted parameters for Gemma
            self.llm = ChatGroq(
                api_key=api_key,
                model_name=model,
                temperature=0.1,
                max_tokens=200,  # Increased to ensure complete responses
                stop=["\n\n", "FINE ANALISI"]  # Added stop tokens for Italian
            )
            
            # Try a simple connection test with format verification
            test_response = self.llm.invoke([
                SystemMessage(content="Respond in this format:\nCLASSIFICAZIONE: VALORE NORMALE\nCONFIDENZA: alta"),
                HumanMessage(content="test")
            ])
            
            if not test_response.content:
                raise ValueError("Empty test response received")
            
            self.logger.info("ChatGroq client initialized and tested successfully")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize ChatGroq client: {str(e)}")
            self.logger.error(f"API key used (first 4 chars): {api_key[:4]}...")
            raise
            
        # Use simpler format reminder specific for Gemma
        self.format_reminder = """
RISPONDI SOLO COSÌ:
CLASSIFICAZIONE: ANOMALIA/VALORE NORMALE
TIPO: [se ANOMALIA] cuscinetti/surriscaldamento/pressione/squilibrio/tensione
CONFIDENZA: alta/media/bassa
INDICATORI: [2-3 valori chiave]
AZIONE: [breve frase]
"""
    ...
    def predict(self, event_time: str, compressor_data_str: str, weather_data_str: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        # Template ultra-conciso che richiede solo classificazione senza spiegazioni
        query_template = """
        Timestamp: {datetime}
        DATI:
        {exact_readings}
        ...
        TIPO: [SOLO se ANOMALIA: cuscinetti/surriscaldamento/pressione/squilibrio/tensione]
        CONFIDENZA: [alta/media/bassa]
        INDICATORI_CHIAVE: [valori chiave]
        RACCOMANDAZIONE: [max 3 parole]
        """
        ...
        try:
            # Add explicit format reminder to each request
            messages = [
                SystemMessage(content=self.format_reminder + "\n" + self.prompt),
            ...
            if not response or not response.content or len(response.content.strip()) < 10:
                raise ValueError(f"Invalid response format: {response.content}")

            # Add more validation for Gemma's response format
            if "CLASSIFICAZIONE:" not in response.content:
                raise ValueError("Response missing required CLASSIFICAZIONE field")
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    try:
                        # Parameters optimized for ultra-concise responses
                        response = self.llm.invoke(
                            messages,
                            temperature=0.1,
                            max_tokens=80,  # Reduced max tokens
                            stop=["CLASSIFICAZIONE:", "SPIEGAZIONE:"],
                            top_p=0.1,
                ...
                # If we reach here, something went wrong but we don't have a specific error to handle
                return fallback
        except Exception as e:
            logging.error(f"Error in _extract_prediction_from_response: {e}")
            return fallback
