...
class LLMPredictor:
    ...
    def __init__(self, api_key: str, model: str = "llama-3.3-70b-versatile"):
        ...
        # Initialize the LLM with explicit API key setting
        self.logger.info(f"Initializing ChatGroq with model: {model}")
        try:
            # Add explicit API key to ensure it's being set correctly
            os.environ["GROQ_API_KEY"] = api_key
            self.llm = ChatGroq(
                api_key=api_key,
                model_name=model,
                groq_api_key=api_key  # Add explicit parameter
            )
    ...
    def predict(self, event_time: str, compressor_data_str: str, weather_data_str: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    # Parameters optimized for llama-3.3-70b-versatile
                    response = self.llm.invoke(
                        messages,
                        temperature=0.1,        # Slightly higher temperature for llama
                        max_tokens=150,         # Keep max tokens at 150
                        stop=["CLASSIFICAZIONE:"],
                        top_p=0.1,             # Higher top_p for llama's capabilities
