...
class LLMPredictor:
    ...
    def __init__(self, api_key: str, model: str = "llama-3.2-90b-vision-preview"):
        ...
        Args:
            ...
            model: LLM model to use, defaults to llama-3.2-90b-vision-preview
        """
        # Configure logging
        ...
        # Optimized rate limiting parameters for 90B model
        self.last_request_time = 0
        self.current_interval = 3.0    # Increased initial interval for larger model
        self.min_interval = 1.5        # Higher minimum interval
        self.max_interval = 60.0       # Higher maximum interval
        self.backoff_factor = 2.0      # More aggressive backoff
        self.success_factor = 0.85     # More conservative reduction
    ...
    def predict(self, event_time: str, compressor_data_str: str, weather_data_str: str, max_retries: int = 3) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    # Optimized parameters for 90B model
                    response = self.llm.invoke(
                        messages,
                        temperature=0.05,        # Lower temperature for more deterministic outputs
                        max_tokens=250,          # Increased for potentially longer responses
                        stop=["CLASSIFICAZIONE:"],
                        top_p=0.05,             # Lower top_p for more focused outputs
