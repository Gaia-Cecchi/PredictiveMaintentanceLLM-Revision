...
def plot_metrics_distribution(metrics_df, output_dir):
    ...
    for metric in main_metrics:
        ...
        # Save the plot
        plt.savefig(os.path.join(output_dir, f'cnn_crossval_{metric}_distribution.png'))
        plt.close()
    ...
    plt.title('Metrics for CNN across all folds (Radar Chart)')
    plt.legend(loc='upper right', bbox_to_anchor=(0.1, 0.1))
    
    plt.savefig(os.path.join(output_dir, 'cnn_crossval_radar_chart.png'))
    plt.close()
...
def create_latex_table(summary_df, output_dir):
    ...
    with open(os.path.join(output_dir, 'cnn_crossval_metrics.tex'), 'w') as f:
        f.write(latex_table)
...
def create_html_report(df, summary_df, main_metrics, output_dir):
    """Creates HTML report"""
    html_path = os.path.join(output_dir, 'cnn_crossval_report.html')
    ...
    # Create HTML content with CNN-specific titles
    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cross-Validation Report for Convolutional Neural Network</title>
    <style>
        body {{
...
<body>
    <h1>Cross-Validation Report for Convolutional Neural Network</h1>
    ...
    <div class="image-container">
        <img src="cnn_crossval_precision_distribution.png" alt="Precision Distribution" class="chart-image">
        <img src="cnn_crossval_recall_distribution.png" alt="Recall Distribution" class="chart-image">
        <img src="cnn_crossval_f1_score_distribution.png" alt="F1-Score Distribution" class="chart-image">
        <img src="cnn_crossval_specificity_distribution.png" alt="Specificity Distribution" class="chart-image">
    </div>
    
    <h3>Metrics Radar Chart</h3>
    <img src="cnn_crossval_radar_chart.png" alt="Metrics Radar Chart" class="full-width-image">
    ...
    <h2>Conclusion</h2>
    <p>The Convolutional Neural Network model shows varying performance in classifying anomalies in the predictive maintenance dataset.</p>
</body>
</html>
"""
...
def main():
    ...
    results_file = os.path.join(base_dir, "test_predictions", "cnn", "results", "cnn_predictions_20250320_104809.csv")
    output_dir = os.path.join(base_dir, "test_predictions", "1) Cross-Validation", "cnn")
    ...
    # Save metrics to CSV
    summary_df.to_csv(os.path.join(output_dir, 'cnn_crossval_summary.csv'), index=False)
    metrics_df.to_csv(os.path.join(output_dir, 'cnn_crossval_fold_metrics.csv'), index=False)
    ...
    # Create reports with CNN-specific titles and content
    report_path = os.path.join(output_dir, 'cnn_crossval_report.md')
    with open(report_path, 'w') as f:
        f.write(f"# Cross-Validation Report for Convolutional Neural Network\n\n")
        ...
        f.write(f"The Convolutional Neural Network model shows varying performance in classifying anomalies in the predictive maintenance dataset.")
