...
def plot_metrics_distribution(metrics_df, output_dir):
    ...
    for metric in main_metrics:
        ...
        # Save the plot
        plt.savefig(os.path.join(output_dir, f'lstm_crossval_{metric}_distribution.png'))
        plt.close()
    ...
    plt.title('Metrics for LSTM across all folds (Radar Chart)')
    plt.legend(loc='upper right', bbox_to_anchor=(0.1, 0.1))
    
    plt.savefig(os.path.join(output_dir, 'lstm_crossval_radar_chart.png'))
    plt.close()
...
def create_latex_table(summary_df, output_dir):
    ...
    with open(os.path.join(output_dir, 'lstm_crossval_metrics.tex'), 'w') as f:
        f.write(latex_table)
...
def create_html_report(df, summary_df, main_metrics, output_dir):
    """Creates HTML report"""
    html_path = os.path.join(output_dir, 'lstm_crossval_report.html')
    ...
    # Create HTML content with LSTM-specific titles
    html_content = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Cross-Validation Report for Long Short-Term Memory Network</title>
    <style>
        body {{
...
<body>
    <h1>Cross-Validation Report for Long Short-Term Memory Network</h1>
    ...
    <div class="image-container">
        <img src="lstm_crossval_precision_distribution.png" alt="Precision Distribution" class="chart-image">
        <img src="lstm_crossval_recall_distribution.png" alt="Recall Distribution" class="chart-image">
        <img src="lstm_crossval_f1_score_distribution.png" alt="F1-Score Distribution" class="chart-image">
        <img src="lstm_crossval_specificity_distribution.png" alt="Specificity Distribution" class="chart-image">
    </div>
    
    <h3>Metrics Radar Chart</h3>
    <img src="lstm_crossval_radar_chart.png" alt="Metrics Radar Chart" class="full-width-image">
    ...
    <h2>Conclusion</h2>
    <p>The Long Short-Term Memory Network model shows varying performance in classifying anomalies in the predictive maintenance dataset.</p>
</body>
</html>
"""
...
def main():
    ...
    results_file = os.path.join(base_dir, "test_predictions", "lstm", "results", "lstm_predictions_20250318_121712.csv")
    output_dir = os.path.join(base_dir, "test_predictions", "1) Cross-Validation", "lstm")
    ...
    # Save metrics to CSV
    summary_df.to_csv(os.path.join(output_dir, 'lstm_crossval_summary.csv'), index=False)
    metrics_df.to_csv(os.path.join(output_dir, 'lstm_crossval_fold_metrics.csv'), index=False)
    ...
    # Create reports with LSTM-specific titles and content
    report_path = os.path.join(output_dir, 'lstm_crossval_report.md')
    with open(report_path, 'w') as f:
        f.write(f"# Cross-Validation Report for Long Short-Term Memory Network\n\n")
        ...
        f.write(f"\n## Interpretation\n\n")
        f.write(f"The metrics show the model's performance across different data splits. ")
        ...
        f.write(f"The Long Short-Term Memory Network model shows varying performance in classifying anomalies in the predictive maintenance dataset.")
