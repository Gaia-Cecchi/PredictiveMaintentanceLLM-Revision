...
def plot_metrics_distribution(metrics_df, output_dir):
    ...
    for metric in main_metrics:
        ...
        # Save the plot
        plt.savefig(os.path.join(output_dir, f'qwen_crossval_{metric}_distribution.png'))
        plt.close()
    
    # Create a radar chart to compare all main metrics
    ...
    plt.title('Metrics for Qwen 2.5-32B across all folds (Radar Chart)')
    plt.legend(loc='upper right', bbox_to_anchor=(0.1, 0.1))
    
    # Save the plot
    plt.savefig(os.path.join(output_dir, 'qwen_crossval_radar_chart.png'))
    plt.close()
...
def create_latex_table(summary_df, output_dir):
    ...
    with open(os.path.join(output_dir, 'qwen_crossval_metrics.tex'), 'w') as f:
        f.write(latex_table)
...
def create_html_report(df, summary_df, main_metrics, output_dir):
    """Creates HTML report"""
    html_path = os.path.join(output_dir, 'qwen_crossval_report.html')
...
<head>
    ...
    <title>Cross-Validation Report for Qwen 2.5-32B</title>
    <style>
        body {{
...
<body>
    <h1>Cross-Validation Report for Qwen 2.5-32B</h1>
    ...
    <div class="image-container">
        <img src="qwen_crossval_precision_distribution.png" alt="Precision Distribution" class="chart-image">
        <img src="qwen_crossval_recall_distribution.png" alt="Recall Distribution" class="chart-image">
        <img src="qwen_crossval_f1_score_distribution.png" alt="F1-Score Distribution" class="chart-image">
        <img src="qwen_crossval_specificity_distribution.png" alt="Specificity Distribution" class="chart-image">
    </div>
    
    <h3>Metrics Radar Chart</h3>
    <img src="qwen_crossval_radar_chart.png" alt="Metrics Radar Chart" class="full-width-image">
    ...
    <h2>Conclusion</h2>
    <p>The Qwen 2.5-32B model shows overall good performance in classifying anomalies in the predictive maintenance dataset.</p>
</body>
</html>
"""
...
def main():
    ...
    # Save metrics to CSV
    summary_df.to_csv(os.path.join(output_dir, 'qwen_crossval_summary.csv'), index=False)
    metrics_df.to_csv(os.path.join(output_dir, 'qwen_crossval_fold_metrics.csv'), index=False)
    ...
    report_path = os.path.join(output_dir, 'qwen_crossval_report.md')
    with open(report_path, 'w') as f:
        f.write(f"# Cross-Validation Report for Qwen 2.5-32B\n\n")
        ...
        # Add specific interpretation based on data
        cv_ratio = summary_df[summary_df['metric'] == 'f1_score']['std'].values[0] / summary_df[summary_df['metric'] == 'f1_score']['mean'].values[0]
        if cv_ratio < 0.05:
        ...
        f.write(f"The Qwen 2.5-32B model shows overall good performance in classifying anomalies in the predictive maintenance dataset.")
