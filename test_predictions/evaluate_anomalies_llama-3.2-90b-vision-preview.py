...
def run_validation(llm_predictor, data_manager, results_dir, sample_size=100, continue_from_backup=False, preloaded_results=None, start_idx=0):
    ...
    # 2. First, ensure all labeled events are included
    ...
    print(f"Including {len(anomalies_df)} anomalies and {len(false_positives_df)} false positives")
    
    # Verifica che il sample_size sia sufficiente
    min_required = len(all_labeled_events)
    if sample_size < min_required:
        print(f"\n⚠️ Warning: Requested sample size ({sample_size}) is less than required minimum ({min_required})")
        print(f"Sample size will be increased to {min_required} to include all labeled events")
        sample_size = min_required
    
    # 3. Calculate how many normal cases to add
    normal_cases_count = sample_size - len(all_labeled_events)
    print(f"\nWill add {normal_cases_count} normal cases to reach target of {sample_size} total cases")
    
    # 4. Select normal cases
...
def main():
    ...
    # Initialize LLM predictor with llama model
    print("Initializing LLM predictor with llama-3.2-90b-vision-preview model...")
    llm_predictor = LLMPredictor(api_key, model="llama-3.2-90b-vision-preview")
    # Replace the prompt with our definitive_prompt and add format instructions
    
    def_prompt = definitive_prompt()
    
    # Formato estremamente minimalista
    format_reminder = """
=== ISTRUZIONE CRITICA ===
RISPONDI SOLO CON:

CLASSIFICAZIONE: [ANOMALIA o VALORE NORMALE]
TIPO: [SOLO SE ANOMALIA]
INDICATORI: [valori rilevanti]
